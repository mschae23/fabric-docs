...
can use `Registries.BLOCK.getCodec()` to get a `Codec<Block>` which serializes to the block identifier and back.
...
- A `Codec<BeanType<?>>` based on this registry. If you use Minecraft's `Registry`, one can be easily made
